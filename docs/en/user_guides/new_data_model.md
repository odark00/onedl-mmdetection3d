...
In this note, we give an example for converting the data into KITTI format, you can refer to this to reorganize your dataset into kitti format. About the standard format dataset, and you can refer to [customize_dataset.md](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/docs/en/advanced_guides/customize_dataset.md).
...
Specifically, we implement a waymo [converter](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/tools/dataset_converters/waymo_converter.py) to convert Waymo data into KITTI format and a waymo dataset [class](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/mmdet3d/datasets/waymo_dataset.py) to process it, in addition need to add a waymo [metric](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/mmdet3d/evaluation/metrics/waymo_metric.py) to evaluate results. Because we preprocess the raw data and reorganize it like KITTI, the dataset class could be implemented more easily by inheriting from KittiDataset. Regarding the dataset evaluation metric, because Waymo has its own evaluation approach, we need further implement a new Waymo metric; more about the metric could refer to [metric_and_evaluator.md](https://github.com/vbti-development/onedl-mmengine/blob/main/docs/en/tutorials/metric_and_evaluator.md). Afterward, users can successfully convert the data format and use `WaymoDataset` to train and evaluate the model by `WaymoMetric`.
...
Suppose we would like to train PointPillars on Waymo to achieve 3D detection for 3 classes, vehicle, cyclist and pedestrian, we need to prepare dataset config like [this](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/configs/_base_/datasets/waymoD5-3d-3class.py), model config like [this](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/configs/_base_/models/pointpillars_hv_secfpn_waymo.py) and combine them like [this](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/configs/pointpillars/pointpillars_hv_secfpn_sbn-all_16xb2-2x_waymoD5-3d-3class.py), compared to KITTI [dataset config](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/configs/_base_/datasets/kitti-3d-3class.py), [model config](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/configs/_base_/models/pointpillars_hv_secfpn_kitti.py) and [overall](https://github.com/VBTI-development/onedl-mmdetection3d/blob/main/configs/pointpillars/pointpillars_hv_secfpn_8xb6-160e_kitti-3d-3class.py).
